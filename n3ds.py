...
if len(sys.argv) < 2:
	...
	class App:
		...
		def store(self, *args):
			...
			IP=self.l_IP.get() 
			f=open("lastIP","w")
			f.write(IP.strip()) #stores text box (as string type)
			f.close()
			root.quit()
...
while done==False:
	...
	for event in pygame.event.get(): # User did something
		...
		if event.type == pygame.JOYAXISMOTION:
			...
			#x c-stick only works if you have a right thumbstick	
			if len(j_axis)>=4:
				if event.axis == j_axis[2] and fabs(event.value) > 0.1 :
					if fabs(event.value)>deadZone:
						server.cstick_coords[0] = int(32767*event.value) #Right_joy_x
					else:
						server.cstick_coords[0] = int(0) #left_joy_x
					server.send()
				if event.axis == j_axis[3] and fabs(event.value) > 0.1 : 
					if fabs(event.value)>deadZone:
						server.cstick_coords[1] = int(-32767*event.value) #Right_joy_y
					else:
						server.cstick_coords[1] = int(0) #left_joy_y
					server.send()
			#ZL/ZR only works here if you have analog triggers
			if len(j_axis)>=6:
				if event.axis == j_axis[4]: #l trig
					if event.value > 0: #more than half depression
